...
async function problemB() {
  ...
  const promises = filenames.map((file) => {
    return promisifiedReadFile(file); // porq necesitamos un array de promesas para pasarle al promise.all
  });

  const stanzas = await Promise.all(promises);

  // for of itera y da un nombre de variable al elemento
  // for (stanza of stanzas) blue(stanza)

  //for each itera ejecutando una funcion por cada elemento
  stanzas.forEach((stanza) => blue(stanza));
